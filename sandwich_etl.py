...
for bucket in s3_bucket.objects.all():
	if today_offset in str(bucket.key) and int(bucket.key.split('%')[1].split(' ')[1].split('-')[0].replace(':', '')) >= time_now_int - 2000 :
...
file_path_collection = glob.glob('/home/ec2-user/tweet_folder/*.txt')
